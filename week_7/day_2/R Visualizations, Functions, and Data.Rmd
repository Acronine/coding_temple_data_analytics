...
cars_mut <-  mutate(mtcars, per_cyl = disp / cyl)
cars_mut
summary(mtcars, weight = mean(weight, na.rm=TRUE))
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    ```{r}
    # Problem 1 I solved in R: Given a string, you have to return a string in which each character (case-sensitive) is repeated once.
    # Write the solution
    #double_char <- function(str){
    #  test <- ""
    #  for (char in strsplit(str, split ="")[[1]]) {
    #    if (char != " "){
    #      (test <- paste(test, char, char, sep=""))
    #    } else {
    #      test <- paste(test, char,sep="")
    #    }
    #  } 
    #  return (test)
    #}

    # Problem 2 I solved in R: The solution would work like the following:
    ...
    #   }
    # }
    # Problem 3 I solved in R:Your Job
    ...
    #     return (0)
    #   }
    #   j <- (seq(from=n,to=(m),by= n))
    #   sum(j[j!= m])
    # }
    # Problem 4 I solved in R:We need a function that can transform a number (integer) into a string.
    # Write the solution
    # number_to_string <- function(n) {
    #  as.character(n)
    # }
    # Problem 5 I solved in R:You get an array of numbers, return the sum of all of the positives ones.
    # Write the solution
...
2.  Create a function that checks an input of type `int` and returns
    fizz if th number is a multiple of 3, buzz if the number is a
    multiple of 5, and fizzbuzz if the number is a multiple of 3 and 5.

    ```{r}
    fb <- function(tege){
      if ((tege %% 3 ==0) & (tege %% 5 ==0)){
        return ("fizzbuzz")
      } else if (tege %% 3 == 0) {
        return ("fizz")
      } else if (tege %% 5 == 0) {
...
-   Girth: Continuous Datatype

-   Height: Continuous Datatype

-   Volume: Continuous Datatype

```{r}
# trees
treedee <- trees
ggplot(data=treedee) +
  geom_point(mapping = aes(Girth, Volume)) +
  geom_smooth(mapping = aes(Girth, Volume))
ggplot(data=treedee) +
  geom_point(mapping = aes(Volume, Height))
ggplot(data=treedee) +
  geom_point(mapping = aes(Height, Girth))
```

Judging by the graphs, it appears that there is definitely a positive
relationship between Girth and Volume, where as height doesn't appear to
have much of relationship with either.

```{r}
ggplot(data=treedee) + 
  geom_density(mapping=aes(Girth))
ggplot(data=treedee) + 
  geom_density(mapping=aes(Volume))
```

Checking the distribution between Girth and Volume, there's a positive
skew for both.
...
```{r}
treefidee <- treedee %>% mutate(
  estimatedAge = (ceiling(Girth / .3)),
  estimatedWeight_ib = (round(pi*(((Girth/2)/12)^2) * Height * 93.6, digits=2))
)
# Reading up online, the average cherry tree grows about .3inches in diameter every year.
# I also found a formula for calculating tree weight in pounds. https://calculator.academy/tree-log-weight-calculator/
treefidee
```

My Hypothesis is there is an interrelated connection among the values
