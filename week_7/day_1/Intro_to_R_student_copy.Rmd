---
title: "Introduction to R: Alex doesn't read this" 
author: "Harrison Crichton"    
date: "February 30th, 1835"
---
...
In R, a package is the same thing as a library in Python. In fact, each library in Python **is** a package. In order to pip install a library, it first needs to be packaged, MADE YOU LOOK!, to [PyPi](https://pypi.org). When we package a library, we are zipping the files together and uploading them as a package to PyPi for anyone to access.
...
inclass <- sprintf("The fire in west %s is %s", area, state)

substr(sub("west", "EAST", inclass), start <- 13, stop <- 16)
...
```{r}
vector22 <- c(43,124,42,56,1)
vector43 <- seq(49,95,by=11)
print(vector22 + vector43)
print(vector22 - vector43)
print(vector22 * vector43)
print(vector22 / vector43)
print(vector22 %/% vector43)
...
1.  Create an array of two 3x3 matrices from the given vectors.
    ...
    Q1 <- array(c(vec1,vec2), dim=c(3,3,2))
    print(Q1)
    ```
...
5.  Create two strings with your first and last name, respectively. Then concatenate them, and print the new value as "My name is \<your newly created concatenated variable will go here\>"
    ...
    first_name <- "Harrison"
    last_name <- "Crichton"
    sprintf("My name is %s.", paste(first_name, last_name))
    ```

6.  What values can a BOOLEAN contain?

    ```{r}
    # TRUE and FALSE are reserved words used to denote boolean.
    ```
